...
"""BGP local preference minus one manipulation test."""
...
class BGPLCLocalPrefMinus1Base(BirdPlanBaseTestCase):
    """Base class for BGP local preference minus one manipulation test."""
    ...
    def _test_announce_routes(self, sim):
        """Announce a route from ExaBGP to BIRD."""
        self._exabgpcli(
            sim,
            "e1",
...
class TestCustomer(BGPLCLocalPrefMinus1Base):
            ...
            "100.64.101.0/24": [
                {
                    ...
                    "attributes": {
                        "BGP.as_path": [65001],
                        "BGP.large_community": [(65000, 8, 1), (65000, 3, 2)],
                        "BGP.local_pref": 749,
...
class TestPeer(BGPLCLocalPrefMinus1Base):
...
class TestTransit(BGPLCLocalPrefMinus1Base):
...
class TestRrclient(BGPLCLocalPrefMinus1Base):
...
class TestRrserver(BGPLCLocalPrefMinus1Base):
...
class TestRrserverRrserver(BGPLCLocalPrefMinus1Base):
...
class TestRoutecollector(BGPLCLocalPrefMinus1Base):
    ...
    def _test_results(self, sim, helpers):
        ...
        # Make sure main BGP tables are empty
        self._check_main_bgp_tables_empty(sim)


class TestRouteserver(BGPLCLocalPrefMinus1Base):
