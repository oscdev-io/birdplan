...
"""
    ...
    def test_as_sets_ipv4_increase(self, sim, tmpdir):
        ...
        if peer_type in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=f"IRR IPv4 network count for peer 'e1' with type '{peer_type}' "
                "increased substantially from previous run: last=36, now=132",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
    ...
    def test_as_sets_ipv6_increase(self, sim, tmpdir):
        ...
        if peer_type in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=f"IRR IPv6 network count for peer 'e1' with type '{peer_type}' "
                "increased substantially from previous run: last=36, now=132",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
    ...
    def test_as_sets_ipv4_decrease(self, sim, tmpdir):
        ...
        if peer_type in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=f"IRR IPv4 network count for peer 'e1' with type '{peer_type}' "
                "decreased substantially from previous run: last=36, now=12",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
