...
"""
    ...
    def test_peeringdb_ipv4_increase(self, sim, tmpdir):
        ...
        if getattr(self, "r1_peer_type") in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=r"IRR IPv4 network count for peer 'e1' increased substantially from previous run: last=36, now=132",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
    ...
    def test_peeringdb_ipv6_increase(self, sim, tmpdir):
        ...
        if getattr(self, "r1_peer_type") in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=r"IRR IPv6 network count for peer 'e1' increased substantially from previous run: last=36, now=132",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
    ...
    def test_peeringdb_ipv4_decrease(self, sim, tmpdir):
        ...
        if getattr(self, "r1_peer_type") in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=r"IRR IPv4 network count for peer 'e1' decreased substantially from previous run: last=36, now=12",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
    ...
    def test_peeringdb_ipv6_decrease(self, sim, tmpdir):
        ...
        if getattr(self, "r1_peer_type") in ("customer", "peer"):
            with pytest.raises(
                BirdPlanError,
                match=r"IRR IPv6 network count for peer 'e1' decreased substantially from previous run: last=36, now=12",
            ):
                super()._birdplan_run(sim, tmpdir, "r1", ["configure"])
