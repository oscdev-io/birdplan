...
"""BGP graceful shutdown inbound from commandline."""
...
"""
    ...
    def test_add_graceful_shutdown(self, sim, tmpdir):
        ...
        self._birdplan_run(sim, tmpdir, "r1", ["bgp", "graceful_shutdown", "add", "r2"])

        # Check r2 was added
        graceful_shutdown_list = self._birdplan_run(sim, tmpdir, "r1", ["bgp", "graceful_shutdown", "list"])
        assert graceful_shutdown_list == ["r2"], "Router r2 is not in the graceful shutdown list"
    ...
    def test_remove_graceful_shutdown(self, sim, tmpdir):
        ...
        self._birdplan_run(sim, tmpdir, "r1", ["bgp", "graceful_shutdown", "remove", "r2"])

        # Check r2 was removed
        graceful_shutdown_list = self._birdplan_run(sim, tmpdir, "r1", ["bgp", "graceful_shutdown", "list"])
        assert graceful_shutdown_list == [], "Router r2 is not in the graceful shutdown list"
